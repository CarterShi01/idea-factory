...
### 9.8 idea-factory 定位为 oc 的"投研部"——agent 服务化第一轮(2026-07-08)

创始人提出终局方向:idea-factory 要成为发现/挖掘/论证/计划四能力的总 Agent 服务,
被 oc(one-creator)直接调用。讨论后钉死边界(详细方案见新文档
`docs/design/agent-service-plan.md`):**idea-factory 管到"可证伪的下注说明书"
为止,不碰实现拆解/派活/执行;outcome 的采集是 oc 的(push),接收与学习(retro/
calibrate)是 idea-factory 自己的架构。** 与 oc 只交换两种边界工件:出=bet memo,
入=outcome 事件。近期只服务"每周批量跑"这一用途(交互式调用推迟,但架构保持
可插)。creator 随后发 `/goal` 指示同会话内按施工票实施,完成 M-A(下注说明书
出口)+ M-B(outcome 收件口)全部、M-C(真信号地基)可完成部分,commits(见本轮
分支 `claude/agent-service-m-a-m-b`)。

**四块落地:**
1. **`ExperimentSpec` 契约**:`Evaluation.experiment` 结构化字段(metric/target/
   kill_below/horizon_days/budget_band),与 `Outcome.prediction` 同构——下注时
   写的赔率就是复盘时对账的口径。gate.py 规则默认 + judge.py LLM 覆盖(judge.json
   system/schema 加 `experiment`,同步 dify 镜像);`enforce_experiment_spec`:
   PURSUE 缺完整规格 → demote REVIEW,与证据门同一纪律。
2. **`bet_memos.json`**(新出向边界工件):portfolio 新模块 `bets.py` 收口 PURSUE/
   REVIEW 幸存者的假设+证据链+实验规格,机读、供 oc 消费;`GET /api/bets` 复用
   `/api/top3` 的 bearer 鉴权;weekly_report 的"测试包"块改从 `experiment` 结构化
   渲染(不再从证据猜定价),顺手修了一处三包重构后遗留的陈旧命令引用。
3. **`POST /api/outcome`**(新入向边界工件):oc 推送赌局结果,`event_id` 幂等
   (重复投递不重复记账),`target` 缺省从 bet memo 自动补;`idea retro` CLI 保留
   为人工版,两条路径共享同一条 `record_outcome`。样本 ≥ min_sample 时 calibrate
   因子相关性摘要附进周报——**执行中发现原方案(portfolio 直接 import calibrate)
   会违反 `test_stage_isolation.py` 钉死的分层铁律**,改为 pipeline.py(唯一允许
   跨段的组合层)预计算后经 `StageContext` 注入,仿照 `ctx.backends` 的既有先例。
4. **真信号地基(部分)**:jobs/marketplace/reviews 三个"钱在流动的地方"源的
   live 路径,从桩(`return []`)改为复用 `vps_browser` 的共享 CDP+Scrapling 机制
   (创始人拍板:统一走这条,不引入 CC-handoff 第二条路)——端到端验证
   `idea run --live --only recall` 真实拉回信号、空 targets 源优雅退化无异常。
   enrich 三 fetcher 的 live 接线**有意留白**:`fetch_via_browser` 产出的是信号
   形状(title/url/category),`Evidence` 需要 keywords/source_date/numbers,这些
   依赖真实目标页结构——没有站点清单前接线是猜测,不是"填函数体"。LLM 默认后端
   切腾讯 router 的决策落成 `scripts/weekly-run.sh` 显式预设,而非静默改
   `idea run` 裸参数默认值——离线默认是项目身份(CLAUDE.md 硬规则),裸调用必须
   继续对 CI/开发者保持零网络零 token。

**验收**:`pytest` 283 passed(本轮前 261,新增 22 条,全部是新功能的直接测试);
`idea run` 裸调用产出字节路径不变;`idea run --live --only recall` 真实跑通。

**创始人待给**:vps_browser 机制的目标站点清单(BOSS直聘/闲鱼成交页/竞品定价页/
评论页等,URL + DOM 选择器,合规创始人判断)——给到后 C1 补 targets、C2 才能真正
接线,是本轮唯一剩下的外部依赖。M-D(异步 job/validate 外部注入/deepdive 定向
挖掘)+ M-E(部署硬化)按创始人决策推迟,详见 `agent-service-plan.md` §0.1/§7。
