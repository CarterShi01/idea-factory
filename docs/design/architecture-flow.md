...
### §9.1 反馈采集(第四张日志 `feedback.jsonl`,人工优化的数据底座)

IdeaLineage 页的 **FeedbackPanel**:对一条 idea 打**问题定位型标签**(错杀/错捧/
信号噪音/伪痛点/证据牵强/不符画像/判对了,多选)+ **`other` 自由文本**。提交时后端
`POST /api/feedback` 调 `idea_lineage` **冻结这条 idea 当时的完整血统快照**(信号→
候选→因子→证据→裁决→prompt/response),自包含地 append 进 `data/ledger/feedback.jsonl`
——即便工件被覆盖/版本被清,这条 case 仍带全部上下文。

**刻意的边界(创始人 2026-07-08 定)**:写入侧常开、结构化;**消费侧是人**——
创始人日后在 CC 里按标签聚合最近的反馈、盘 case、决定怎么改 prompt/阈值/因子,
手写代码。**不在前端触发任何自动优化、不自动回流改配置**。这与 `calibrate` 的
"只读给建议、人工拍板" 是同一取舍,只是把粒度从"权重相关性"下沉到"逐条带血统的
case"。标签词表 = 漏斗失败点,聚合时一眼知道该修哪段。
