"""Tests for the ``idea-factory rank`` CLI subcommand."""
...
def isolated_ranks_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("idea_factory.api.DEFAULT_RANKS_PATH", ranks_path)
    return ranks_path
...
def test_rank_cli_persists_override(isolated_ranks_path, capsys):
    ...
    refreshed = {
        idea["id"]: idea["rank"]
        for idea in _load_mock_ideas(ranks_path=isolated_ranks_path)
    }
    assert refreshed[idea_id] == 4

    sorted_ideas = list_ideas(
        sort="rank", ideas=_load_mock_ideas(ranks_path=isolated_ranks_path)
    )
    assert sorted_ideas[0]["rank"] >= 4
...
def test_rank_cli_rejects_out_of_range(isolated_ranks_path, capsys, bad_rank):
    ...
    captured = capsys.readouterr()
    assert captured.out == ""
    assert "rank must be an integer" in captured.err
