...
from idea_core.factors import has_hard_anti_fit
...
) -> tuple[list[IdeaCandidate], dict[str, str]]:
    """Hard-kill candidates the founder profile itself flags as anti-fit.

    Reuses ``idea_core.factors.has_hard_anti_fit`` (the single source of truth
    for the founder's anti-fit vocabulary) -- a bare idea that just doesn't
    mention any channel is NOT killed here (that's merely unvalidated, and
    ``founder_fit``/``distribution_fit`` will rank it low downstream); only
    candidates with **explicit** anti-fit language and no offsetting
    monopoly/referral channel or lang-region moat are gated out.
    """
    kept: list[IdeaCandidate] = []
    killed: dict[str, str] = {}
    for c in candidates:
        if has_hard_anti_fit(c):
            killed[c.id] = PROFILE_MISMATCH
            continue
        kept.append(c)
    return kept, killed
