...
from idea_factory.runtime.config import load_sources
from .channels import REGISTRY, CollectContext, ensure_loaded
...
def collect_all(
    ...
    单源失败被隔离(不影响其它源)。
    """
    from idea_factory.contract.models import SOURCE_PERSONA

    ensure_loaded()
    data_dir = Path(data_dir)
    raw_dir = data_dir / "raw"
    cache_dir = data_dir / "cache"
    cfg = config or load_sources()
