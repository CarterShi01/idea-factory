...
永不触网。``live=True`` 复用 vps_browser 的共享 CDP+Scrapling 机制(agent-service-plan.md
M-C1,创始人 2026-07-08 拍板"live 一律走 vps_browser"——不新写爬虫):挂到已登录的
VPS Chrome,访问 ``config/sources.json`` 本段的 ``targets``(闲鱼成交页等,当前留空,
等创始人给站点清单 + DOM 选择器,见该文档 §5-①)。无 targets 时优雅返回 ``[]``。
"""
...
class MarketplaceAdapter:
    ...
    def collect(self, ctx: CollectContext) -> list[dict]:
        if ctx.live:
            return fetch_via_browser(ctx.config, source=self.source, source_name_default="marketplace")
